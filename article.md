...
In our example from above it would take [0.5, 2] as it's intial input. Then the subsequent hidden layers are optimized to try match the original image as much as possible. It does so by using something called reconstruction error, which measures how different the autoencoder’s output is from its original input.

Even though the encoder and decoder have a shared loss, the weights of each network are updated to do two separate things. 

**Encoder-Decoder Summary**

_Encoder weights_: Learn to compress images → "What is the best way to represent this image in a smaller dimensional space? [0.5, 2]?"

_Decoder weights_: "What sort of image comes out of [0.5, 2] →  [1...784]"
...
The rational behind autoencoders for anomaly detection comes from understanding one of it's greatest pitfalls, which is, the more noise present in an image, the worse the reconstruction error. Therefore, if you have a set of anomalous images that include unnecessary noise, or anomalies that would not normally be present in these images, there should be a higher reconstruction error. Then you can use the average reconstruction error of the anomalous images as a threshold to identify if there is an anomaly present. 

Some articles have also suggested using the kernel density estimation within the latent space in addition to the reconstruction error to help identify potential anomalies within images.  
...
I used the https://github.com/google-deepmind/dsprites-dataset and manipulated the sprites to have different background colors so that we can also see how this method works in images with more than just one channel. Most previous articles tend to work with only black and white images and most video game assets will have 3 channels. 

![alt text](./article_imgs/image-1.png)

![alt text](./article_imgs/image-2.png)

This set of images with the background transformed, became my ground truth dataset, and from there I injected different randomly sized and colored elipses into the images as potential "anomalies". 
...
A pytorch dataset stores the data and their corresponding labels all in one object. It also creates minibatches of the data for each epoch at training. 
...
One question you might ask yourself when looking at this AutoEncoder, is why does the size of the channels increase in the encoder layer and decrease in the decoder layer? The reason for this has to do with the way the actual Conv2d layer of a neural network works. 
